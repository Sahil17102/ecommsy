...
function buildColorVars(colors: ColorPalette): Record<string, string> {
  return {
    ...
    // tailwind.config.js exposes these as `error`/`error-bg`/`error-border`.
    // Without them every `text-error` in the app fell back to currentColor,
    // which is why error states rendered black instead of red.
    "--color-error": colors.danger,
    "--color-error-bg": colors.dangerBg,
    "--color-error-border": colors.dangerBorder,
  };
}
